...
* @file       Tests.cpp
*
* @brief      Test routines for Facoin.
*
...
Test_GenerateGroupParams()
{
	...
	for (count = 0; count < 1; count++) {
		...
		try {
			group = deriveIntegerGroupParams(calculateSeed(GetTestModulus(), "test", FACOIN_DEFAULT_SECURITYLEVEL, "TEST GROUP"), pLen, qLen);
...
Test_ParamGen()
{
	...
	try {
		...
		Params testParams(GetTestModulus(),FACOIN_DEFAULT_SECURITYLEVEL);
...
Test_RunAllTests()
{
	printf("FaCoin v%s self-test routine\n", FACOIN_VERSION_STRING);
	...
	// Run through all of the Facoin tests
