...
* @file       Params.h
*
* @brief      Parameter classes for Facoin.
*
...
public:
	/** @brief Construct a set of Facoin parameters from a modulus "N".
	* @param N                A trusted RSA modulus
	* @param securityLevel    A security level expressed in symmetric bits (default 80)
	*
	* Allocates and derives a set of Facoin parameters from
...
public:
	...
	**/
	Params(Bignum accumulatorModulus,
	       uint32_t securityLevel = FACOIN_DEFAULT_SECURITYLEVEL);
