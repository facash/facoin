...
///
/// \brief      Parameter manipulation routines for the Facoin cryptographic
///             components.
///
...
/// \brief Fill in a set of Facoin parameters from a modulus "N".
...
///
/// \throws         FacoinException if the process fails
///
...
/// be known to any party, or the security of Facoin is
...
CalculateParams(Params &params, Bignum N, string aux, uint32_t securityLevel)
{
	...
	if (NLen < 1023) {
		throw FacoinException("Modulus must be at least 1023 bits");
	}

	// Verify that "securityLevel" is  at least 80 bits (minimum).
	if (securityLevel < 80) {
		throw FacoinException("Security level must be at least 80 bits.");
	}
	...
	*pLen = *qLen = 0;
	...
	if (securityLevel < 80) {
		...
		*pLen = 3072;
		...
		throw FacoinException("Security level not supported.");
	}

	if (*pLen > maxPLen) {
		throw FacoinException("Modulus size is too small for this security level.");
	}
}
...
deriveIntegerGroupParams(uint256 seed, uint32_t pLen, uint32_t qLen)
{
	...
	if ((uint32_t)(result.modulus.bitSize()) < pLen ||          // modulus is pLen bits long
		...
		throw FacoinException("Group parameters are not valid");
	}
...
deriveIntegerGroupFromOrder(Bignum &groupOrder)
{
	...
	throw FacoinException("Too many attempts to generate Schnorr group.");
}
                              ...
                              uint256 *resultPseed, uint256 *resultQseed)
{
	...
	if (qLen > (sizeof(seed)) * 8) {
		...
		// throw FacoinException("Seed is too short to support the required security level.");
	}
	...
	*resultGroupOrder = generateRandomPrime(qLen, seed, &qseed, &qgen_counter);
	...
	// Throw an exception.
	throw FacoinException("Unable to generate a prime modulus for the group");
}
...
/// \throws                             A FacoinException if error.
///
...
calculateGroupGenerator(uint256 seed, uint256 pSeed, uint256 qSeed, Bignum modulus, Bignum groupOrder, uint32_t index)
{
	...
	if (index > 255) {
		throw FacoinException("Invalid index for group generation");
	}
	...
	throw FacoinException("Unable to find a generator, too many attempts");
}
                    ...
                    uint32_t *prime_gen_counter)
{
	...
	if (primeBitLen < 2) {
		throw FacoinException("Prime length is too short");
	}
	...
	if (primeBitLen < 33) {
		...
		throw FacoinException("Unable to find prime in Shawe-Taylor algorithm");
	...
	throw FacoinException("Unable to generate random prime (too many tests)");
}
