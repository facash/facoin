...
{
    QString ret = QString("facoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non FAC unit input in lnReqAmount, we generate the URI with FAC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(FacoinUnits::format(FacoinUnits::FAC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
